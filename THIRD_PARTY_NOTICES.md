...
The [license proposal](LICENSE_STATUS.md) concerns original project material. It does not change the terms of dependencies, font software or cited works.
...
The stored records have a [source-member manifest](data/record_bundle_manifest.json). It records origins and byte identities, not copyright ownership. The author must confirm authority to license the original study records, software, prose and artwork, including historical material. Actual editable external composite-schematic sources remain unavailable and have not been cleared for distribution. See [availability limits](docs/REPRODUCIBILITY_LIMITS.md).
