...
def draw_protocol(ax, df, protocol: str, color: str, title: str, show_y: bool) -> None:
    ...
    for run in ['primary','replication']:
        ...
        yerr = np.vstack([
            y - finite['ci_low'].to_numpy(float),
            finite['ci_high'].to_numpy(float) - y,
        ])
        ax.errorbar(
            x, y, yerr=yerr, fmt=style['marker'], ms=5.0,
        ...
        ax.errorbar(
            [0.0], [y0], yerr=y0err, fmt=style['marker'], ms=5.3,
            mfc=style['mfc'], mec=style['mec'], mew=1.0,
    ...
    ticks = [0.0, 1/256, 1/128, 1/64, 1/32]
    labels = [r'$\infty$', r'$256$', r'$128$', r'$64$', r'$32$']
