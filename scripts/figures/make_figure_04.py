...
def distance_panel(distance_csv,pdf,png):
    ...
    ax.axhline(0,color=FIG_GRAY,lw=0.72,zorder=0); ax.grid(axis='y',color=FIG_GRID,lw=0.55,zorder=0)
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--distance-csv',type=Path,required=True); ap.add_argument('--contrast-csv',type=Path,required=True); ap.add_argument('--distance-pdf',type=Path,required=True); ap.add_argument('--distance-png',type=Path,required=True); ap.add_argument('--contrast-pdf',type=Path,required=True); ap.add_argument('--contrast-png',type=Path,required=True); a=ap.parse_args(); configure(); distance_panel(a.distance_csv,a.distance_pdf,a.distance_png); contrast_panel(a.contrast_csv,a.contrast_pdf,a.contrast_png)
