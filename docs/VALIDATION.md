...
2. `reproduce_core_records.py` recalculates core point estimates from the original observations, verifies the accepted Figure 1 table against its original hybrid source, and reconstructs Figures 3/4 intervals from their preserved bootstrap arrays. It does not claim that the missing accepted Figure 1 resamples were recovered. The output labels those distinctions.
