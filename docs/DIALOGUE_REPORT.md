...
|---|---|---|
| [M1](#m1)–[M2](#m2) | Identify the information being controlled and define the response | Definitions |
| [M3](#m3) | Establish the fixed-spectrum contrast | 1 |
| [M4](#m4)–[M5](#m5) | Identify the exact mechanism and measured redistribution | 2 |
| [M6](#m6)–[M7](#m7) | Remove spectrum replacement and examine size/replication | 3 |
| [M8](#m8) | Isolate the measurement-location intervention | 4 |
| [M9](#m9) | State the surviving conclusion and its limits | Synthesis |

The appendix answers implementation and inference objections in the same order. The questions represent reader perspectives, not quotations or attributed opinions of individual researchers. S1–S10 refer to the local [source register](#source-register-and-reproduction); each entry identifies its authoritative evidence or method.
...
**Next question.** [Which response makes that distinction both precise and computable?](#m2)
...
Here $\mathbb E_U$ holds the input pure trajectory state fixed. Averaging this response across prepared trajectories is a separate step; restricting those trajectories to a central-spectrum stratum is a further selection. The density matrix obtained by forgetting the measurement record and mixing trajectories is not the input to this calculation. [S2, S3]
...
**Next question.** [Does a monitoring-dependent difference remain when the complete central spectrum is actually controlled?](#m3)
...
**Next question.** [Can the response difference be explained by identifiable state information rather than a vague appeal to Schmidt vectors?](#m4)
...
**Next question.** [How does monitoring change these exact response coordinates in the physical ensemble?](#m5)
...
![Figure 2, first panel. Exact response alphabet.](../figures/core/figure_02_response_matrix.png)

![Figure 2, second panel. Conditional probability slopes.](../figures/core/figure_02_redistribution_matrix.png)

<a id="figure-2-caption"></a>
...
**Follow Figure 2.** [Canonical code table](../data/processed/core_figures/figure_02_boundary_codes.csv) · [plot script](../scripts/figures/make_figure_02.py) · [caption](#figure-2-caption) · [claim M2](CLAIM_EVIDENCE_MAP.md) · [exact alphabet and decomposition](THEORY.md#3-finite-alphabet-theorem) · [conditional estimator](NUMERICAL_METHODS.md#6-exact-spectrum-fixed-effect-estimator). The displayed probability coefficients use unmodified physical states at fixed size and protocol, controlling time and central spectrum; monitoring probability varies on the observed support.
...
**Next question.** [Is this mechanism visible in physical states without replacing their central eigenvalues?](#m6)
...
**Answer.** A stabilizer reduced density matrix has $2^{S_m}$ equal nonzero eigenvalues, each $2^{-S_m}$, and all other eigenvalues zero. At fixed $n$, equality of $S_m$ therefore fixes the complete central spectrum, including its rank and multiplicities. Naturally generated monitored stabilizer states can be compared within fixed $(n,\tau,S_m)$ strata without altering any state. Equality of entropy would not fix the full spectrum for general states; the flat-spectrum property is essential here. [S2, S3]
...
**Next question.** [Does the physical fixed-spectrum difference survive larger systems and new random seeds?](#m7)
...
**Follow Figure 3.** [Canonical size table](../data/processed/core_figures/figure_03_size_scaling.csv) · [plot script](../scripts/figures/make_figure_03.py) · [caption](#figure-3-caption) · [claims M3–M4 and limit L1](CLAIM_EVIDENCE_MAP.md) · [estimator](NUMERICAL_METHODS.md#6-exact-spectrum-fixed-effect-estimator) · [locked fit](NUMERICAL_METHODS.md#7-predeclared-finite-size-model). Each coefficient controls time and central spectrum within a size and protocol; the figure then compares sizes, protocols, and disjoint seed runs.
...
**Next question.** [Can one controlled measurement-location change reproduce the response suppression while preserving the spectrum?](#m8)
...
![Figure 4, first panel. Spectrum-preserving distance profile without a fitted curve.](../figures/core/figure_04_distance_decay.png)

![Figure 4, second panel. Paired conditional and unconditional contrasts.](../figures/core/figure_04_conditioning_contrast.png)

<a id="figure-4-caption"></a>
...
**Follow Figure 4.** [Distance table](../data/processed/core_figures/figure_04_distance_decay.csv) and [contrast table](../data/processed/core_figures/figure_04_conditioning_contrast.csv) · [plot script](../scripts/figures/make_figure_04.py) · [adopted full caption](FIGURE_BASELINE.md#replacement-figure-4-caption) · [claim M5, corollary T1, and limit L2](CLAIM_EVIDENCE_MAP.md) · [pairing and eligibility](NUMERICAL_METHODS.md#10-paired-measurement-location-intervention). The pre-state and preceding circuit history are held fixed between copies; the single measurement location changes. The two panels show different endpoints, not two estimates of the same quantity.
...
**Next question.** [What single conclusion can the four figures support together?](#m9)
