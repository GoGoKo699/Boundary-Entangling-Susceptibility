...
|---|---|---|
| Natural logarithms, Eq.1 and Rényi definition, p.5 | $S_j$ in bits: $-\operatorname{Tr}\rho_{[1:j]}\log_2\rho_{[1:j]}$ | Divide the review entropy by $\ln2$ to express it in bits. For stabilizers, $P_j=2^{-S_j}$. |
...
| $S_{2A}$ and $e^{-S_{2A}}=\operatorname{Tr}\rho_A^2$, Eq.13, p.16 | $P_j$ for purity; $L_m=D(1-P_m)/(D-1)$ for normalized linear entropy | $\chi_2=\mathbb E_U[L'_m-L_m]$ and $\chi_{\rm rel}=\chi_2/P_m$ are finite purity responses, not finite logarithmic entropy increments. |
