...
**Status: the repository is public, verified on 2026-09-14. Release preparation and the available-history exposure review are complete. No blocking disclosure was identified in the reviewed material.** The adopted licenses apply to the material in this tree. The owner approved integration and public visibility. This page records the review scope, findings, and [integration and publication status](#integration-and-publication).
...
|---|---|
| License | Adopted: the owner approved MIT for original software and CC BY 4.0 for original documentation, data and figures, and confirmed authority under Ruge Lin's name. Both permit commercial reuse. See the [adoption record](#license-adoption). |
...
| Review and stability | The existing `test`, `recorded-evidence` and `figure1-uncertainty` checks must pass on the selected commit; integration results are recorded below. At the exposure inspection, the branch endpoint reported `main` as unprotected. A pull-request/check requirement can be considered before accepting public contributions. No settings were changed during preparation. |
| Citation and version | Version `0.1.0` and planned release date `2026-09-14` are prepared in `CITATION.cff`. Confirm the date when publishing the first tagged release; no tag or GitHub Release existed at this preparation step. Add a DOI or preprint reference only after one has actually been issued. |
...
Licensing grants reuse rights independently of repository visibility. No history, branch, discussion, workflow run or artifact was deleted during preparation.
...
During that exposure-review follow-up, hashes of all 619 starting tracked files were saved before editing. Only this readiness page changed, and one compact coverage JSON was added. All 618 other files, including scientific code, tables, accepted figures, analysis plans and historical records, retained their hashes. The reader/navigation self-test, existing data-integrity verification and whitespace check were rerun; the PR carries the actual CI status. No archived program was executed for exposure inspection.
...
On 2026-09-12 the owner approved the proposed material split and confirmed authority to license the included original material under Ruge Lin's name. The adoption adds root `LICENSE` and `LICENSES/CC-BY-4.0.txt`, updates the current scope, README, contribution and data notices, and declares MIT for software package metadata. The first licensed snapshot was prepared on a private working branch; its material scope is defined by the notice carried in that tree.
...
<a id="remaining-release-steps"></a>
## Integration and publication

On 2026-09-12 the owner approved proceeding with the two reviewed PRs and public visibility, with the known author-email and historical-preview exposure described above. [Reader-route PR #11](https://github.com/GoGoKo699/Boundary-Entangling-Susceptibility/pull/11) entered `main` at `be36f6d5a6a031f3d755d0fbaa88b58ef0dadf62`; its tree exactly matches the reviewed reader-route tree. [Release-preparation PR #12](https://github.com/GoGoKo699/Boundary-Entangling-Susceptibility/pull/12) supplies the licenses, release records and final integration wording. Its GitHub record identifies the final head, merge commit and CI results.

The owner has made the repository public. GitHub reported public visibility on 2026-09-14, with `main` at `9c9945132ffaded220703b561ab9d18f2420a2c9`. The earlier private-status statements in the dated exposure and license-adoption records describe those inspection snapshots.

The first tagged release is being prepared as `v0.1.0`, matching the package version. [CITATION.cff](../CITATION.cff) now carries version `0.1.0` and the planned release date `2026-09-14`. This metadata update does not create a tag or GitHub Release. If the tagged release is published on another date, update `date-released` before tagging. No release DOI or associated preprint is declared.

The dated exposure snapshot and protected scientific material remain unchanged. Live inspection of the public README, equations, figure paths and citation panel remains outstanding; the completed source checks do not substitute for it. The unavailable external editable composite sources remain an explicitly excluded asset-completion gap; the included numerical workflows do not depend on them.
