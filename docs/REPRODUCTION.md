...
This recalculates the ten Figure 1 held-out point estimates and verifies its accepted confidence-interval table against the recovered hybrid source. Exact accepted Figure 1 resamples were not recovered; see the reproducibility limits. It reconstructs Figure 2's representative code-probability slopes from state records, all 16 Figure 3 finite-size coefficients, and both Figure 4 endpoint estimates and distance means. It separately checks plotted archived intervals against the stored original arrays/tables. The output says which values were newly calculated and which interval calculations reuse archived resamples.

## 3. Repeat the September evidence reassessment
...
## 4. Materialize a research workspace and run simulator checks
